...
class AiSocialLogicTest extends TestCase
{
    ...
    public function test_post_rate_limit_switches_to_comment()
    {
        ...
        $this->assertDatabaseMissing('ai_reactions', [
            'user_id' => $user->id,
        ]);
    }
    ...
    public function test_nothing_action_does_not_like()
    {
    ...
    public function test_comment_ratio_forces_new_post()
    {
        $user = $this->createTestUser(['nome' => 'RatioUser', 'energia_sociale' => 100]);
        $otherUser = $this->createTestUser(['nome' => 'OtherUser', 'energia_sociale' => 0]);

        $lastPost = AiPost::create(['user_id' => $otherUser->id, 'content' => 'Latest post']);
        AiPost::whereKey($lastPost->id)->update([
        ...
        $deciderMock = Mockery::mock(AiActionDeciderService::class);
        $deciderMock->shouldReceive('decideAction')->never();
        $this->instance(AiActionDeciderService::class, $deciderMock);

        $aiServiceMock = Mockery::mock(AiService::class);
        $aiServiceMock->shouldReceive('generateJson')
            ->once()
            ->andReturn(['content' => 'Post by ratio']);
        $aiServiceMock->shouldReceive('getLastLog')
            ->andReturn(null);
        $this->instance(AiService::class, $aiServiceMock);
        ...
        $this->assertDatabaseHas('ai_posts', [
            'user_id' => $user->id,
            'content' => 'Post by ratio',
        ]);
    }

    public function test_ai_failure_falls_back_to_like()
    {
        $user = $this->createTestUser(['nome' => 'FallbackUser', 'energia_sociale' => 100]);
        $otherUser = $this->createTestUser(['nome' => 'OtherUser', 'energia_sociale' => 0]);

        $commentTarget = AiPost::create(['user_id' => $otherUser->id, 'content' => 'Target post']);
        $likeTarget = AiPost::create(['user_id' => $otherUser->id, 'content' => 'Like target']);

        $targetSelectorMock = Mockery::mock(AiTargetSelectorService::class);
        $targetSelectorMock->shouldReceive('findPostsToComment')
            ->andReturn(new \Illuminate\Database\Eloquent\Collection([$commentTarget]));
        $targetSelectorMock->shouldReceive('findPostsToLike')
            ->andReturn(new \Illuminate\Database\Eloquent\Collection([$likeTarget]));
        $this->instance(AiTargetSelectorService::class, $targetSelectorMock);

        $deciderMock = Mockery::mock(AiActionDeciderService::class);
        $deciderMock->shouldReceive('decideAction')->andReturn('COMMENT_POST');
        $this->instance(AiActionDeciderService::class, $deciderMock);

        $aiServiceMock = Mockery::mock(AiService::class);
        $aiServiceMock->shouldReceive('generateJson')
            ->once()
            ->andThrow(new \Exception('Failed to parse JSON response: syntax error'));
        $aiServiceMock->shouldReceive('getLastLog')
            ->andReturn(null);
        $this->instance(AiService::class, $aiServiceMock);
        ...
        $this->assertDatabaseHas('ai_reactions', [
            'user_id' => $user->id,
            'target_id' => $likeTarget->id,
            'reaction_type' => 'like',
        ]);

        $this->assertDatabaseMissing('ai_comments', [
            'user_id' => $user->id,
        ]);
    }

    public function test_comment_selection_prefers_older_posts_when_enabled()
    {
        $user = $this->createTestUser(['nome' => 'Picker']);
        $otherUser = $this->createTestUser(['nome' => 'Other']);

        $oldPost = AiPost::create(['user_id' => $otherUser->id, 'content' => 'Old post']);
        AiPost::whereKey($oldPost->id)->update([
            'created_at' => now()->subHours(4),
            'updated_at' => now()->subHours(4),
        ]);

        $recentPost = AiPost::create(['user_id' => $otherUser->id, 'content' => 'Recent post']);
        AiPost::whereKey($recentPost->id)->update([
            'created_at' => now()->subHour(),
            'updated_at' => now()->subHour(),
        ]);

        $service = new class(app(\App\Services\AiAffinityService::class)) extends AiTargetSelectorService
        {
        ...
        $posts = $service->findPostsToComment($user, 5);

        $this->assertTrue($posts->contains('id', $oldPost->id));
        $this->assertFalse($posts->contains('id', $recentPost->id));
    }

    public function test_comment_selection_falls_back_to_recent_posts_when_no_old_posts()
    {
        $user = $this->createTestUser(['nome' => 'Picker']);
        $otherUser = $this->createTestUser(['nome' => 'Other']);
        ...
        $this->assertTrue($posts->contains('id', $recentPost->id));
    }
    ...
    public function test_force_new_post_after_20_null_events()
    {
        ...
        // 3. Mock Decider to return NOTHING (should be overridden)
        $deciderMock = Mockery::mock(AiActionDeciderService::class);
        $deciderMock->shouldReceive('decideAction')->andReturn('NOTHING');
