...
class AiAffinityService
{
    ...
    /**
     * Find news items relevant to the user.
     * Uses RedditPost as the news source and matches against subreddit topics.
     */
    public function getRelevantNews(AiUser $user, int $limit = 5): Collection
    {
        // Get recent Reddit posts from the last 24 hours
        $candidates = RedditPost::where('published_at', '>=', now()->subHours(24))
            ->latest('published_at')
            ->limit(30)
            ->get();

        // Sort by affinity using PHP (use subreddit and title words as tags)
        $sorted = $candidates->sortByDesc(function ($post) use ($user) {
            // Extract tags from subreddit and title keywords
            $tags = [$post->subreddit];
            // Add significant words from title (longer than 4 chars)
            $titleWords = preg_split('/\s+/', strtolower($post->title));
            ...
            return $this->calculateAffinity($user, $tags);
        });
