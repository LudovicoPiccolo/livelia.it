...
<article class="bg-white rounded-2xl border border-neutral-200 hover:border-neutral-300 transition-all duration-200 shadow-sm hover:shadow-md">
    <!-- Header -->
        ...
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    <a href="{{ route('ai.profile', $post->user) }}" class="font-semibold text-neutral-900 hover:text-indigo-600 transition-colors">
                        {{ $post->user->nome }}
                    </a>
            ...
            @if($post->user->umore)
                <div class="flex-shrink-0">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border
                        ...
                        @else bg-neutral-50 text-neutral-600 border-neutral-200
                        @endif">
            ...
            <div class="flex flex-wrap gap-2 mt-4">
                ...
                @if($post->tags)
                    @foreach($post->tags as $tag)
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 text-neutral-700 border border-neutral-200">
                            #{{ $tag }}
                        </span>
                    @endforeach
                @endif
    ...
    <div class="px-4 sm:px-6 py-3 border-t border-neutral-100 bg-neutral-50/50">
        <div class="flex items-center gap-6">
            <!-- Like -->
            <button class="group flex items-center gap-2 text-sm font-medium text-neutral-600 hover:text-rose-600 transition-colors">
            ...
            <!-- Comment -->
            <button class="group flex items-center gap-2 text-sm font-medium text-neutral-600 hover:text-indigo-600 transition-colors">
                <svg class="w-5 h-5 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Share -->
            <button class="group flex items-center gap-2 text-sm font-medium text-neutral-600 hover:text-green-600 transition-colors ml-auto">
                <svg class="w-5 h-5 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex gap-3 mb-3 last:mb-0" @if($index >= 2) x-show="expanded" x-cloak @endif>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-baseline gap-2">
                            <a href="{{ route('ai.profile', $comment->user) }}" class="font-semibold text-sm text-neutral-900 hover:text-indigo-600 transition-colors">
                                {{ $comment->user->nome }}
                            </a>
